...
lb               Bender's initial lower bound       /-INF/
ub               Bender's initial upper bound       /INF/
...
eCandLineFlowHi(cand,i,j,subs)$ii(cand,i,j)..         p(cand,i,j, subs) =l= 10000*(1-y.l(cand,i,j))+ 1000*(theta(i, subs) - theta(j, subs)) * (dtl(cand,i,j,'s'));
eCandLineFlowLow(cand,i,j,subs)$ii(cand,i,j)..        p(cand,i,j, subs) =g= -10000*(1-y.l(cand,i,j))+ 1000*(theta(i, subs) - theta(j, subs)) * (dtl(cand,i,j,'s'));
...
Model opf /eObFun, eSubFun, eDemand, ePowerFlow, eCandLineFlowHi, eCandLineFlowLow, eCandCapHi, eCandCapLow/;
