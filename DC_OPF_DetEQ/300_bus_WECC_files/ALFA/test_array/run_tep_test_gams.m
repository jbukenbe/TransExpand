...
pTime.name = 'pTime';
pTime.type = 'parameter';
pTime.form = 'full';
pTime.uels = {'t1'};
pTime.val = problem.run_time_lim;
...
tic
if problem.samp_method == "LF"
    wgdx(filename, subs, psub_w, psub_mean, global_mean, pload, pRenew, pVarCost, pTime);
    if problem.opt_mod == "bender"
        system ('gams "lf_tep_bender" lo=2');
    elseif problem.opt_mod == "MIP"
        system ('gams "lf_tep_MIP" lo=3');
    end
elseif problem.samp_method == "IS"
    wgdx(filename, subs, psub_w, pload, pRenew, pVarCost, pTime);
    system ('gams "tep_MIP" lo=3');
else
    wgdx(filename, subs, psub_w, pload, pRenew, pVarCost, pTime);
    if problem.opt_mod == "bender"
