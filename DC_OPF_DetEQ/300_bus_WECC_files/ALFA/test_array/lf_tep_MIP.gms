...
eObFun..                               z =e= global_mean('m1') + (sum(subs, psub_w(subs)*(zs(subs)- psub_mean(subs))))/8736+ sum((cand,i,j)$ii(cand,i,j),y(cand,i,j)*dtl(cand,i,j,'cost'));
...
z.stage = 0;
x.stage(g,subs) = ORD(subs);
p.stage(l,i,j, subs)$ii(l,i,j) = ORD(subs);
theta.stage(i, subs) = ORD(subs);
pns.stage(i, subs) = ORD(subs);
sg.stage(g, subs)= ORD(subs);
zs.stage(subs)= ORD(subs);
