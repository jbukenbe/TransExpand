...
%2          11/29/2018 JesseB   Working version
    ...
    methods
        ...
        function [obj, opt_out] = solve(obj)
            ...
            [sub_list,sub_lookup_list,subs_idx] = unique(obj.tstnep_params.gexp_run_map);
...
% currently not needed?
            ...
            sg.val = [];
            for sub_idx = 1:length(sub_lookup_list)
                r_idx = sub_lookup_list(sub_idx);
                new_gens = obj.tstnep_params.gexp_gens(r_idx,:)'.*gen_cand;
                new_gens(new_gens == 0) = [];
